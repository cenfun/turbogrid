    ...
    <div class="api-container">
      ...
      >
        ...
        <section>
          ...
          <div>只有正向（非反向）模式参与计分，并使用固定整数权重：</div>
          <ul>
            <li>每个至少匹配一列的正向模式加 100 分。</li>
            <li>每个被模式匹配的列加 20 分。</li>
            <li>每组“模式 + 列”取质量最高的一次匹配：完整字段匹配加 20 分，字段前缀匹配加 10 分，单词边界匹配加 5 分。</li>
            <li>质量最高的匹配与模式大小写完全一致时，额外加 2 分。</li>
            <li>重复匹配最多加 2 分：第二次和第三次匹配各加 1 分，更多匹配不再加分。</li>
            <li>在每一列中，连续的两个正向模式首次匹配位置按输入顺序排列且不重叠时，加 5 分。</li>
            <li>反向模式只影响该行是否匹配，不参与计分。</li>
          </ul>
          <div>
            计算只使用已有的匹配范围、固定整数加法和首次匹配顺序判断，不统计整个数据集，也不会额外
            扫描行。例如，模式 "Foo" 完整匹配字段 "Foo" 时得 142 分；模式 "Foo Bar" 匹配字段
            "Foo Bar" 时得 264 分，其中包含 200 分模式匹配、40 分列匹配、10 分前缀匹配、5 分单词
            边界匹配、4 分大小写完全一致和 5 分顺序奖励。
          </div>
          <pre><code class="language-js">
                            grid.setOption({
                                highlightKeywords: {
