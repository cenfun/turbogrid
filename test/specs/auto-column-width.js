...
describe('autoColumnWidth', function() {
    ...
    // Use column ids that do NOT collide with default columnTypes mappings
    // (e.g. 'name' maps to type 'tree' which sets width=230)
    const baseColumns = [
        { id: 'col1', name: 'Name' },
    ...
    it('default autoColumnWidth is false', async () => {
        await initRender({}, { columns: baseColumns, rows });
        assert.equal(grid.getOption('autoColumnWidth'), false);
    });

    // ====================================================================

    it('distributes extra space to auto-sized columns proportionally', async () => {
        await initRender(
        ...
        const base1 = grid.getComputedColumnWidth(col1);
        const base3 = grid.getComputedColumnWidth(col3);
        assert(col1.tg_width > base1, 'col1 got extra space');
        assert(col3.tg_width > base3, 'col3 got extra space');
    });

    // ====================================================================

    it('excludes columns with explicit width', async () => {
        await initRender(
            { autoColumnWidth: true },
            { columns: baseColumns, rows }
        );

        assert.equal(grid.getColumnItemById('col2').tg_width, 80);
    });
    ...
    it('clamps widths to maxWidth and puts leftover in blank column', async () => {
        ...
        assert(colA.tg_width <= 100, 'A clamped');
        assert(colB.tg_width <= 100, 'B clamped');
    ...
    it('reset logic restores auto-sized columns to base widths', async () => {
        ...
        // Run the reset logic from resizeHandler
    ...
    it('redistributes correctly at different container sizes', async () => {
        ...
        // Distribute for 400px container
        ...
        const autoCols = [];
        let totalW = 0;
        columns.slice(0, -1).forEach(function(c) {
            if (!Util.isSize(c.width)) {
                autoCols.push(c);
        ...
        let extra = grid.containerWidth - grid.columnsWidth;
        if (extra > 0 && autoCols.length > 0) {
            let dist = 0;
            const res = autoCols.map(function(c) {
                const e = Math.floor(extra * (c.widthWeight || 1) / totalW);
                dist += e;
                return { column: c, extra: e };
            });
            res[res.length - 1].extra += extra - dist;
            res.forEach(function(r) { r.column.tg_width += r.extra; });
        }
        ...
        // Reset and distribute for 1200px
        for (let i = 0, l = columns.length - 1; i < l; i++) {
            const c = columns[i];
            if (!Util.isSize(c.width)) {
                c.tg_width = grid.getComputedColumnWidth(c);
        ...
        extra = grid.containerWidth - grid.columnsWidth;
        if (extra > 0 && autoCols.length > 0) {
            let dist = 0;
            const res = autoCols.map(function(c) {
                const e = Math.floor(extra * (c.widthWeight || 1) / totalW);
    ...
    it('works with frozen columns', async () => {
        ...
        assert(col1.tg_width >= grid.getComputedColumnWidth(col1),
            'frozen col1 got extra');
        assert(col3.tg_width >= grid.getComputedColumnWidth(col3),
            'non-frozen col3 got extra');
    });
